...
async function main() {
  ...
  console.log("Creating DAO on Sepolia with account:", deployer.address);
  
  const balance = await deployer.provider.getBalance(deployer.address);
  console.log("Account balance:", ethers.formatEther(balance));

  try {
    // Get the deployed DAOFactorySimple contract on Sepolia
    const daoFactoryAddress = "0xB5bD8645bA6614aF4782F0212155dD33F8e66269"; // Sepolia address
    ...
    const daoName = "Sepolia Test DAO";
    const tokenSymbol = "SEP";
    ...
    if (daoCreatedEvent) {
      ...
      console.log("\n🎉 DAO Created Successfully on Sepolia!");
